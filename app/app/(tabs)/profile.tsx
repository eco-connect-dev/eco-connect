    ...
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <ContactRow editable={isEditing} icon="mail-outline" keyboardType="email-address" label="Email Address" onChangeText={(email) => setDraft((current) => ({ ...current, email }))} value={isEditing ? draft.email : user.email} />
          <View style={styles.divider} />
          <ContactRow editable={isEditing} icon="call-outline" label="Phone Number" keyboardType="phone-pad" onChangeText={(phone) => setDraft((current) => ({ ...current, phone }))} value={isEditing ? draft.phone : user.phone} />
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.divider} />
          <InfoRow editable={isEditing} label="Location" location onChangeText={(location) => setDraft((current) => ({ ...current, location }))} value={isEditing ? draft.location : user.location} />
        </View>

        <View style={styles.card}>
          <Text style={styles.cardHeading}>ACTIVITY SUMMARY</Text>
          <View style={styles.activityRow}>
            <ActivityStat count={String(user.reportedIssues)} label="Reported Issues" style={styles.issueStat} />
...
function ContactRow({ editable, icon, keyboardType, label, onChangeText, value }: { editable: boolean; icon: "mail-outline" | "call-outline"; keyboardType?: "email-address" | "phone-pad"; label: string; onChangeText: (value: string) => void; value: string }) {
  return <View style={styles.contactRow}><View style={styles.iconBox}><Ionicons name={icon} size={15} color={Colors.forestGreen} /></View><View style={styles.contactCopy}><Text style={styles.fieldLabel}>{label}</Text>{editable ? <TextInput autoCapitalize="none" autoCorrect={false} keyboardType={keyboardType} onChangeText={onChangeText} style={[styles.fieldValue, styles.inlineInput]} value={value} /> : <Text style={styles.fieldValue}>{value}</Text>}</View></View>;
}

function InfoRow({ editable = false, label, location = false, onChangeText, value }: { editable?: boolean; label: string; location?: boolean; onChangeText?: (value: string) => void; value: string }) {
  const valueStyle = [styles.infoValue, location && styles.locationValue];
  return <View style={styles.infoRow}><Text style={styles.infoLabel}>{label}</Text>{editable ? <TextInput autoCapitalize="words" onChangeText={onChangeText} style={[...valueStyle, styles.infoInput]} value={value} /> : <Text style={valueStyle}>{value}</Text>}</View>;
}
